...
export default function ChatScreen() {
  ...
  const roomId = chatApi.buildRoomId(user._id || user.userId, facultyId);
  ...
  return (
    ...
    >
      ...
      <View className="px-4 pt-12 pb-4 bg-card border-b border-border flex-row items-center">
        ...
        <View>
          <Text className="text-white font-bold">{facultyName}</Text>
          <Text className="text-muted text-xs">Faculty</Text>
        </View>
      </View>
      ...
      {loading ? (
        ...
        >
          {messages.length === 0 && (
            <Text className="text-muted text-center mt-10 text-sm">
          ...
          {messages.map((msg, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  <Text className={`${isMe ? 'text-white' : 'text-slate-200'} text-sm leading-5`}>
                    {msg.message}
                  </Text>
                </View>
      ...
      <View className="flex-row items-center px-4 py-3 bg-card border-t border-border">
        ...
        <TouchableOpacity
          ...
          className={`ml-3 w-11 h-11 rounded-full items-center justify-center ${
            text.trim() ? 'bg-primary' : 'bg-surface border border-border'
          }`}
        >
          {sending ? (
            <ActivityIndicator color="white" size="small" />
          ) : (
            <Ionicons name="send" size={18} color={text.trim() ? 'white' : '#64748b'} />
          )}
        </TouchableOpacity>
      </View>
    </KeyboardAvoidingView>
